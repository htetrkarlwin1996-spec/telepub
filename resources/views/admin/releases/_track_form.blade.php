    ...
    <div class="border-2 border-black p-4 credit-section" data-field="primary_artists" data-track="{{ $index }}">
        ...
        <div class="person-list space-y-2">
            @if(count($primaryArtists) > 0)
                ...
                @endforeach
            @else
                <!-- Default: pre-fill with the album artist name -->
                <div class="person-entry flex items-center gap-2" data-person-idx="0">
                    <input type="text"
                        ...
                        class="flex-1 px-3 py-2 border-2 border-black font-bold text-sm focus:outline-none focus:ring-0 focus:border-brand-500"
                        placeholder="Artist name">
                    <input type="hidden" name="tracks[{{ $index }}][primary_artists][0][spotify_url]" value="{{ $artist->spotify_profile_url ?? '' }}" class="link-spotify">
                    <input type="hidden" name="tracks[{{ $index }}][primary_artists][0][apple_music_url]" value="{{ $artist->apple_music_profile_url ?? '' }}" class="link-apple_music">
                    <input type="hidden" name="tracks[{{ $index }}][primary_artists][0][youtube_url]" value="{{ $artist->youtube_profile_url ?? '' }}" class="link-youtube">
                    <input type="hidden" name="tracks[{{ $index }}][primary_artists][0][tidal_url]" value="{{ $artist->tidal_profile_url ?? '' }}" class="link-tidal">
                    <button type="button" onclick="openLinksModal(this)"
                        class="px-3 py-2 bg-gray-100 border-2 border-black font-extrabold text-[10px] uppercase hover:bg-brand-100 transition-all links-btn">
                        Links
                    </button>
