    ...
    <div class="min-h-screen bg-[#FFF8E7]">
        <div class="max-w-3xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
            ...
            <div class="mb-8">
                ...
                <p class="text-sm font-bold text-black/60 text-center mt-1">
                    @if(isset($album))
                        Editing: <span class="text-brand-600">{{ $album->title }}</span>
                    @else
                        Create a new release for any artist
                    @endif
                </p>
            </div>
            ...
            <form method="POST" action="{{ isset($album) ? route('admin.releases.update-step1', $album) : route('admin.releases.store-step1') }}" enctype="multipart/form-data" class="space-y-6">
                @csrf
                @if(isset($album)) @method('PUT') @endif

                <!-- Artist Selection -->
                <div class="bg-white border-2 border-black shadow-[5px_5px_0px_0px_rgba(0,0,0,1)] p-6">
                    <label class="block font-extrabold text-sm uppercase mb-2">Artist <span class="text-red-500">*</span></label>
                    <p class="text-xs font-bold text-black/50 mb-2">Select the artist for this release.</p>
                    <select name="artist_id" required
                        ...
                        @foreach($artists as $artist)
                            <option value="{{ $artist->id }}"
                                {{ old('artist_id', isset($album) ? $album->artist_id : '') == $artist->id ? 'selected' : '' }}>
                                {{ $artist->artist_name }} ({{ $artist->user->email ?? 'no email' }})
                            </option>
                        @endforeach
                    </select>
                </div>
                ...
                <script>
                    function addCollaborator() {
                        ...
                        const artists = @json($artists->map(fn($a) => ['id' => $a->id, 'name' => $a->artist_name, 'email' => $a->user->email ?? 'no email']));
                        ...
                        artists.forEach(a => {
                            options += `<option value="${a.id}">${a.name} (${a.email})</option>`;
                        });
                ...
                <div class="bg-white border-2 border-black shadow-[5px_5px_0px_0px_rgba(0,0,0,1)] p-6">
                    ...
                    <input type="text" name="title" value="{{ old('title', isset($album) ? $album->title : '') }}" required
                        class="w-full px-4 py-3 border-2 border-black font-bold text-sm focus:outline-none focus:ring-0 focus:border-brand-500"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="bg-white border-2 border-black shadow-[5px_5px_0px_0px_rgba(0,0,0,1)] p-6">
                        ...
                        <select name="release_type" required class="w-full px-4 py-3 border-2 border-black font-bold text-sm focus:outline-none focus:ring-0 focus:border-brand-500">
                            <option value="single" {{ old('release_type', isset($album) ? $album->release_type : '') === 'single' ? 'selected' : '' }}>Single</option>
                            <option value="ep" {{ old('release_type', isset($album) ? $album->release_type : '') === 'ep' ? 'selected' : '' }}>EP</option>
                            <option value="album" {{ old('release_type', isset($album) ? $album->release_type : '') === 'album' ? 'selected' : '' }}>Album</option>
                        </select>
                    </div>
                    ...
                    <div class="bg-white border-2 border-black shadow-[5px_5px_0px_0px_rgba(0,0,0,1)] p-6">
                        ...
                        <select name="genre" required class="w-full px-4 py-3 border-2 border-black font-bold text-sm focus:outline-none focus:ring-0 focus:border-brand-500">
                            ...
                            @foreach($genres as $genre)
                                <option value="{{ $genre }}" {{ old('genre', isset($album) ? $album->genre : '') === $genre ? 'selected' : '' }}>{{ $genre }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                ...
                <div class="bg-white border-2 border-black shadow-[5px_5px_0px_0px_rgba(0,0,0,1)] p-6">
                    ...
                    <input type="date" name="release_date" value="{{ old('release_date', isset($album) && $album->release_date ? $album->release_date->format('Y-m-d') : '') }}" required
                        class="w-full px-4 py-3 border-2 border-black font-bold text-sm focus:outline-none focus:ring-0 focus:border-brand-500">
                </div>
                ...
                <div class="bg-white border-2 border-black shadow-[5px_5px_0px_0px_rgba(0,0,0,1)] p-6">
                    ...
                    <p class="text-xs font-bold text-black/50 mb-3">JPG or PNG · Max 10MB @if(!isset($album)) · Required @else · Leave empty to keep current @endif</p>
                    <div class="border-2 border-dashed border-black p-8 text-center" id="upload-area">
                        ...
                        <p class="text-sm font-bold text-black/60 mb-2">
                            @if(isset($album) && $album->cover_art)
                                Current: <span class="text-brand-600">{{ basename($album->cover_art) }}</span>
                            @else
                                Click or drag to upload
                            @endif
                        </p>
                        <p class="text-xs font-bold text-black/40" id="file-name">
                            @if(isset($album) && $album->cover_art)
                                No file selected (keep current)
                            @else
                                No file selected
                            @endif
                        </p>
                        <input type="file" name="cover_art" id="cover_art" accept=".jpg,.jpeg,.png" {{ !isset($album) ? 'required' : '' }}
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="bg-white border-2 border-black shadow-[5px_5px_0px_0px_rgba(0,0,0,1)] p-6">
                        ...
                        <input type="text" name="copyright_holder" value="{{ old('copyright_holder', isset($album) ? $album->copyright_holder : '') }}" required
                            class="w-full px-4 py-3 border-2 border-black font-bold text-sm focus:outline-none focus:ring-0 focus:border-brand-500"
                    ...
                    <div class="bg-white border-2 border-black shadow-[5px_5px_0px_0px_rgba(0,0,0,1)] p-6">
                        ...
                        <input type="text" name="phonogram_right_holder" value="{{ old('phonogram_right_holder', isset($album) ? $album->phonogram_right_holder : '') }}" required
                            class="w-full px-4 py-3 border-2 border-black font-bold text-sm focus:outline-none focus:ring-0 focus:border-brand-500"
                            placeholder="e.g. Star Records">
                    </div>
                </div>
                ...
                <div class="flex items-center justify-between">
                    <a href="{{ route('admin.releases') }}" class="px-6 py-3 bg-gray-200 border-2 border-black font-extrabold text-sm uppercase hover:bg-gray-300 transition-all">
                        Cancel
                    </a>
