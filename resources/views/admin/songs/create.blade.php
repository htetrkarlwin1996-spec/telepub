<x-app-layout>
    <x-slot name="header">
        <h2 class="font-black text-2xl text-black tracking-tight">{{ __('Create Song') }}</h2>
    </x-slot>
            ...
            <div class="bg-white border-2 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] p-6">
                <form method="POST" action="{{ route('admin.songs.store') }}" enctype="multipart/form-data">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <select id="album_id" name="album_id" class="block mt-1 w-full border-2 border-black px-3 py-2.5 text-sm font-semibold text-black focus:border-brand-500 focus:ring-0 focus:shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] transition-all rounded-none" required>
                                ...
                                <option value="{{ $album->id }}">{{ $album->title }} — {{ $album->artist->artist_name ?? 'Unknown' }}</option>
                                @endforeach
                            </select>
                    ...
                    <div class="flex justify-end mt-4 gap-3">
                        ...
                        <x-primary-button>{{ __('Create Song') }}</x-primary-button>
                    </div>
                </form>
            </div>
        </div>
